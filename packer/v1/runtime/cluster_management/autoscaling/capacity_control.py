...
def main(cloud_interface: CloudInterface, os_interface: OSInterface) -> int:
    ...
    print(
        f"Maximum: {cloud_capacity.maximum_nodes} nodes ->"
    ...
    print(
        f"Desired: {cluster_capacity.desired_workers} workers ->"
        f" {desired_nodes_requested} nodes"
    )
    cloud_issue = False
    if any(
        desired_nodes_requested != n
        for n in (cloud_capacity.desired_nodes, cloud_capacity.current_nodes)
    ):
