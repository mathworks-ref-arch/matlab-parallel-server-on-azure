...
def main(
    ...
    if autoshutdown_schedule.startswith("After"):
        # If autoshutdown_schedule is in the form 'After x hours', calculate the termination time
    ...
    if current_time > autoshutdown_timestamp:
        ...
        print(
            f"> Autoshutdown schedule not reached. Time left before termination: {int(total_minutes)} minute(s). Exiting..."
        )
